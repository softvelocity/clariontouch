      ...
      IF SELF.FindPt (pt.ID) = 0
        ...
        IF i <> 0
          ...
          xy.y = pt.ptlocal.y
            ...
            !SELF.FEQs[i] = CREATE (0, CREATE:Ellipse)
            ...
            IF TechData = TRUE
                ...
                LOOP Ls# = 10 TO 1 BY -1
                    GET(Starts,Rs#)
                    ?Starts{PROP:Text} = ?Starts{PROP:Text} & Starts.StartPoint & '<13,10>'  !Let's load up the TEXT control with only 10 elements, Full Touch Support involves 10 points only
                    Rs# -= 1
                END !LOOP            
            END !IF
